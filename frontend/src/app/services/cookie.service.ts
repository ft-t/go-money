...
export class CookieService {
  ...
  hasJwt(): boolean {
    return !!this.get(CookieInstances.Jwt, null);
  }
   ...
   */
  get(name: string, val: any = null): string {
    if (val || this.documentIsAccessible && this.check(name)) {
      name = encodeURIComponent(name);

      const regExp: RegExp = this.getCookieRegExp(name);
      // @ts-ignore
      const result: RegExpExecArray = regExp.exec(val ? val : this.document.cookie);
  ...
  set(name: string, value: string, expires?: number | Date,
      path?: string, domain?: string, secure?: boolean, sameSite?: 'Lax' | 'None' | 'Strict'): void;
