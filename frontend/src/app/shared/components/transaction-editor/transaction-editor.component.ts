...
export class TransactionEditorComponent implements OnInit, OnChanges {
    ...
    @Input() public transaction: Transaction = create(TransactionSchema, {});
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes['transaction']) {
            ...
            this.form = this.buildForm(changes['transaction'].currentValue);
        }
    }

    async refresh() {
        this.form = this.buildForm(this.transaction); // has previous value
    }

    getTitle(): string {
        if (!this.shouldShowTitle) return '';
    ...
    async search(event: AutoCompleteCompleteEvent) {
        try {
            let resp = await this.transactionService.getTitleSuggestions(
                create(GetTitleSuggestionsRequestSchema, {
                    limit: 50,
                    query: event.query
                })
            );
            ...
            if (!this.titleAutoComplete || this.titleAutoComplete.length == 0) this.titleAutoComplete = [event.query];
        } catch (e) {
    ...
    getSourceAccountName(): string {
        const accountId = this.form.get('sourceAccountId')?.value;

        if (accountId && this.allAccounts[accountId]) {
            return this.allAccounts[accountId].name;
        }

        return accountId ? `Account #${accountId}` : 'Unknown';
    }

    getDestinationAccountName(): string {
        const accountId = this.form.get('destinationAccountId')?.value;
        ...
        switch (this.form.get('type')!.value) {
            case TransactionType.INCOME:
                req.transaction.value = create(IncomeSchema, {
                    ...
                    destinationAmount: NumberHelper.toPositiveNumber(destinationAmount),
                    destinationCurrency: destinationCurrency,

                    sourceAccountId: sourceAccountId,
                    sourceAmount: NumberHelper.toNegativeNumber(sourceAmount),
            ...
            case TransactionType.EXPENSE:
                req.transaction.value = create(ExpenseSchema, {
                    ...
                    fxSourceAmount: NumberHelper.toNegativeNumber(fxSourceAmount)
                });
