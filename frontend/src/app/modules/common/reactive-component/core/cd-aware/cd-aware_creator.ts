...
export function createCdAware<U>(cfg: {
  ...
  const potentialObservablesSubject: Subject<ObservableInput<U> | undefined | null> = new Subject();
  const observablesFromTemplate$: Observable<ObservableInput<U> | undefined | null>
  ...
  const rendering$ = observablesFromTemplate$.pipe(
    ...
    switchMap((observable$) => {
      ...
      // - null - null passed directly or no value set over `ngrxPush` pipe
      if (observable$ == null) {
