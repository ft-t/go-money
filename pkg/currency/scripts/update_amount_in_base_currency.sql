    ...
    select t.id,
           case
               when t.source_currency = 'USD' then
                   t.source_amount
               when t.destination_amount is not null and t.destination_currency = 'USD' -- if other side already in usd, no need to convert
                   then
           ...
           case
               when t.destination_currency = 'USD' then
                   t.destination_amount
               when t.source_amount is not null and t.source_currency = 'USD' then  -- if other side already in usd, no need to convert
                   t.source_amount
               when t.destination_currency != 'USD' then
